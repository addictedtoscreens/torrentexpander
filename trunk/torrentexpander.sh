...
if [ "$imdb_title" ] && [ "$imdb_funct_on" == "yes" ] && [ "$subtitles_mode" != "yes" ]; then
	...
	if [[ "$wget_curl" == *wget* ]]; then
		...
		xml_cont=`echo "$("$wget_curl" -q "http://dedi-fr-30107.op-net.com/imdbWebService.php?m=$imdb_title&o=xml" -O -; wait)"`;
	elif [[ "$wget_curl" == *curl* ]]; then
		# Using curl to fetch data if available
		xml_cont=`echo "$("$wget_curl" -silent -i "http://dedi-fr-30107.op-net.com/imdbWebService.php?m=$imdb_title&o=xml"; wait)"`;
	fi
	
	# Adding XML path to the debug log
	if [[ "$debug_mode" == "yes" ]]; then echo "IMDB XML URL: http://dedi-fr-30107.op-net.com/imdbWebService.php?m=$imdb_title&o=xml" >> "$debug_log"; fi
	if [ "$xml_cont" ]; then
